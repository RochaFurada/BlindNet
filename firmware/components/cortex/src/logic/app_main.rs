...
use crate::platform::swarm_agent::{SwarmAgentConfigRaw, SwarmFrameRaw};
use crate::platform::telemetry_agent::TelemetryAgentConfigRaw;
use crate::platform::wifi_manager::WifiManagerConfigRaw;
use crate::platform::zone_firewall::{ZoneFirewallConfigRaw, ZoneFirewallDecisionRaw};
use crate::platform::zone_gateway::ZoneGatewayConfigRaw;
use crate::platform::{
...
fn enter_normal_mode(cfg: &ZoneguardConfigRaw) -> Result {
    ...
    let fw_config = ZoneFirewallConfigRaw {
        zone_id: cfg.zone_id,
        default_allow: true,
    ...
    let gateway_config = ZoneGatewayConfigRaw {
    ...
    let swarm_config = SwarmAgentConfigRaw {
    ...
    let telemetry_config = TelemetryAgentConfigRaw {
        zone_id: cfg.zone_id,
        guardian_id: cfg.guardian_id,
        interval_ms: 5_000,
...
fn test_zone_firewall_fake_flow() -> Result {
    ...
    let mut decision: ZoneFirewallDecisionRaw = zeroed_raw();
...
unsafe extern "C" fn on_swarm_frame(frame: *const SwarmFrameRaw, _ctx: *mut c_void) {
    if frame.is_null() {
        return;
    }
